...
def titlecase(text, callback=None, small_first_last=True):
    ...
    for line in lines:
        ...
        for word in words:
            ...
            if match:
                tc_line.append(
                    "%s%s" % (match.group(1).capitalize(), titlecase(match.group(2), callback, small_first_last))
                )
                continue
            ...
            if "/" in word and "//" not in word:
                slashed = map(lambda t: titlecase(t, callback, False), word.split("/"))
                tc_line.append("/".join(slashed))
                continue

            if "-" in word:
                hyphenated = map(lambda t: titlecase(t, callback, small_first_last), word.split("-"))
...
def cmd():
    ...
    in_group.add_argument("string", nargs="*", default=[], help="String to titlecase")
    in_group.add_argument("-f", "--input-file", help="File to read from to titlecase")
    parser.add_argument("-o", "--output-file", help="File to write titlecased output to)")
